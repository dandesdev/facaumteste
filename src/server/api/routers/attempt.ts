...
export const attemptRouter = createTRPCRouter({
  ...
  start: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        !evaluation ||
        !["published", "active"].includes(evaluation.status ?? "")
      ) {
        throw new TRPCError({
          code: "NOT_FOUND",
  ...
  submitResponse: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Update or Insert response (upsert pattern)
      // Note: schema has unique("response_attempt_item_unique") on (attemptId, itemId)
